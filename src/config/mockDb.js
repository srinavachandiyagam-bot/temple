...
class MockDatabase {
  ...
  async query(text, params = []) {
    ...
    if (/^INSERT INTO registrations/i.test(normalized)) {
      ...
      // Ensure aliases
      if (record.cashfree_order_id && !record.cf_order_id) record.cf_order_id = record.cashfree_order_id;
      if (record.cashfree_order_id && !record.order_id) record.order_id = record.cashfree_order_id;
      if (record.cf_order_id && !record.cashfree_order_id) record.cashfree_order_id = record.cf_order_id;
      if (record.order_id && !record.cashfree_order_id) record.cashfree_order_id = record.order_id;
    ...
    // 3. SELECT FROM registrations WHERE cashfree_order_id = $1 (or cf_order_id or order_id)
    if (/SELECT .* FROM registrations WHERE (cashfree_order_id|cf_order_id|order_id) = \$1/i.test(normalized)) {
      const orderId = params[0];
      const match = this.registrations.find(r => r.cashfree_order_id === orderId || r.cf_order_id === orderId || r.order_id === orderId);
      if (match) {
        const copy = { ...match, order_id: match.cashfree_order_id || match.order_id, cf_order_id: match.cashfree_order_id || match.cf_order_id };
        return { rows: [copy], rowCount: 1 };
      }
      return { rows: [], rowCount: 0 };
    }

    // Also handle SELECT with cashfree_order_id in WHERE with extra conditions (e.g., pending check)
    if (/FROM registrations WHERE cashfree_order_id = \$1/i.test(normalized) && normalized.includes('WHERE cashfree_order_id')) {
      const orderId = params[0];
      const match = this.registrations.find(r => r.cashfree_order_id === orderId || r.cf_order_id === orderId || r.order_id === orderId);
      return { rows: match ? [{ ...match, order_id: match.cashfree_order_id, cf_order_id: match.cashfree_order_id }] : [], rowCount: match ? 1 : 0 };
    }
    ...
    if (/SELECT .* FROM registrations/i.test(normalized) && /ORDER BY/i.test(normalized) || /^SELECT .* FROM registrations\s*$/i.test(normalized) || (/SELECT .* FROM registrations/i.test(normalized) && !normalized.includes('WHERE'))) {
      // Only handle if it's a plain select all (not the specific WHERE cases already handled)
      if (!normalized.includes('WHERE registration_id') && !normalized.includes('WHERE cashfree_order_id') && !normalized.includes('WHERE id =') && !normalized.includes('COUNT(')) {
        const copy = [...this.registrations].reverse().map(r => ({ ...r, order_id: r.cashfree_order_id || r.order_id, cf_order_id: r.cashfree_order_id || r.cf_order_id }));
    ...
    if (/^UPDATE registrations SET/i.test(normalized)) {
      ...
      if (/WHERE id = \$\d+/i.test(whereClause)) {
        ...
        if (rec) targets = [rec];
      } else if (/WHERE cashfree_order_id = \$\d+/i.test(whereClause) || /WHERE cf_order_id = \$\d+/i.test(whereClause) || /WHERE order_id = \$\d+/i.test(whereClause)) {
        const m = whereClause.match(/WHERE (cashfree_order_id|cf_order_id|order_id) = \$(\d+)/i);
        const orderVal = m ? getParam(m[2]) : params[params.length - 1];
        const rec = this.registrations.find(r => r.cashfree_order_id === orderVal || r.cf_order_id === orderVal || r.order_id === orderVal);
        if (rec) targets = [rec];
        ...
        if (rec) targets = [rec];
      } else if (/WHERE cashfree_order_id = \$1/i.test(normalized) && !setClause.includes('$1')) {
        const orderId = params[0];
        const rec = this.registrations.find(r => r.cashfree_order_id === orderId);
        if (rec) targets = [rec];
      }
      if (targets.length === 0 && params.length >= 1) {
        // Fallback: try last param as identifier
        const possibleOrderId = params[params.length - 1];
        let rec = this.registrations.find(r => r.cashfree_order_id === possibleOrderId || r.id === Number(possibleOrderId));
        if (rec) targets = [rec];
      }
      if (targets.length > 0) {
        ...
        for (const part of setParts) {
          ...
          if (valExpr.startsWith('$')) {
            ...
            if (col === 'cashfree_order_id' && val) targets.forEach(t => { t.cf_order_id = val; t.order_id = val; });
            if (col === 'cf_order_id' && val) targets.forEach(t => { t.cashfree_order_id = val; t.order_id = val; });
            if (col === 'order_id' && val) targets.forEach(t => { t.cashfree_order_id = val; t.cf_order_id = val; });
